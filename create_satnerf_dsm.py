...
def create_pretty_dsm(run_id, logs_dir, output_dir, epoch_number, checkpoints_dir=None, root_dir=None, img_dir=None, gt_dir=None):
    ...
    print('run_id, logs_dir, checkpoints_dir, epoch_number-1: ', run_id, logs_dir, checkpoints_dir, epoch_number-1)
    ...
    if args.gt_dir is not None:
        ...
        mae = sat_utils.compute_mae_and_save_dsm_diff(pred_dsm_path, src_id, args.aoi_id, args.gt_dir, out_dir, epoch_number)
        print("Path to output NeRF DSM: {}".format(pred_dsm_path))
        print("Altitude MAE: {:.3f}".format(np.nanmean(mae)))
    ...
    print('create_satnerf_dsm finished !')

if __name__ == '__main__':
    args = Test_parser()

    create_pretty_dsm(args.run_id, args.logs_dir, args.output_dir, args.epoch_number)
