...
class NeRF_pl(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        if self.depth:
            ...
            self.depth_loss = DepthLoss(lambda_ds=args.ds_lambda, GNLL=args.GNLL, usealldepth=args.usealldepth, margin=args.margin, stdscale =args.stdscale)
            self.ds_drop = np.round(args.ds_drop * args.max_train_steps)
        self.define_models()
        self.outdir = "{}/{}/".format(args.logs_dir, args.exp_name)
        ...
        if self.args.beta == True:
            if self.args.model == "sat-nerf" or self.args.model == "sps-nerf":
    ...
    def forward(self, rays, ts, mode='test', valid_depth=None, target_depths=None, target_std=None):
        ...
        for i in range(0, batch_size, chunk_size):
            rendered_ray_chunks = \
                render_rays(self.models, self.args, rays[i:i + chunk_size],
                            ts[i:i + chunk_size] if ts is not None else None, mode=mode, valid_depth=valid_depth, target_depths=target_depths, target_std=target_std)
    ...
    def training_step(self, batch, batch_nb):
        ...
        depths = None
        target_std = None
        if self.args.model == 'sps-nerf':
            valid_depth = batch["color"]["valid_depth"] # (B)
            depths = batch["color"]["depths"] # (B,2)
            target_std = batch["color"]["depth_std"]  #(B)
            #rays_ref = batch["color"]["rays_ref"] # (B, 8)
            #print("batch_nb, depths.shape, target_std.shape: ", batch_nb, depths.shape, target_std.shape)

        if self.args.ds_noweights:
            depths[:, 1] = torch.ones_like(depths[:, 1])

        results = self(rays, ts, mode='train', valid_depth=valid_depth, target_depths=depths, target_std=target_std)
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if self.args.model == "sat-nerf" or self.args.model == "sps-nerf":
            ...
            ts = None
        results = self(rays, ts, mode='test')
        loss, loss_dict = self.loss(results, rgbs)
        ...
        if True:
            ...
            if batch_nb != 0:
                ...
                try:
                    ...
                    mae_ = compute_mae_and_save_dsm_diff(out_path, batch["src_id"][0], aoi_id,
...
def main():
    ...
    shutil.copyfile(args.root_dir+"/train.txt", system.outdir+"/train.txt")
    shutil.copyfile(args.root_dir+"/test.txt", system.outdir+"/test.txt")
